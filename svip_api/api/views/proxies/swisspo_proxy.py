...
def swisspo_request(request, path):
    ...
    target_url = 'https://swiss-po.ch/%s' % path
...
#  1) POST to https://swiss-po.ch/includes/main/fetch_data_pdb.php {get_option: BRAF} to get HTML dropdown of PDBs for this gene
#  2) POST to https://swiss-po.ch/includes/main/fetch_data_link.php {get_option: BRAF} to get links to uni/nexprot for this gene
#  3) POST to https://swiss-po.ch/includes/main/fetch_data_pdb_table.php {option_protein: BRAF, option_threshold: 9} to get
#     some kind of response to put in a "select PDB" panel that we never actually see...?
#  4) POST to https://swiss-po.ch/includes/main/fetch_data_fasta.php {get_option: BRAF} to get the name of the FASTA file
#  5) POST to https://swiss-po.ch/includes/main/fetch_residueNumbers.php {option_pdbId: 5HIE, option_chain: A} to get
...
#  6) POST to https://swiss-po.ch/includes/main/fetch_data_pdb_info3D.php {get_option: 5HIE} to get an HTML blob
#     about the PDB to display in the tooltip above NGL.
#  7) GET https://swiss-po.ch/data/BRAF/MUSCLE_BRAF_ordered_aligned.fasta to get FASTA data (using name from step 4)
#  8) GET https://swiss-po.ch/data/BRAF/PDB/5HIE.A.prot_corrected.pdb to get PDB (using name from step 5)
...
def get_pdbs(request, protein):
    ...
    """
    response = requests.post('https://swiss-po.ch/includes/main/fetch_data_pdb.php', data={
        'get_option': protein
    })
...
def get_residues(request, pdb_id, chain):
    ...
    :return: an array of {pdb, chain} objects identifying the RCSB PDB ID and relevant chains for that protein
    """
    response = requests.post('https://swiss-po.ch/includes/main/fetch_residueNumbers.php', data={
...
def get_pdb_data(request, pdb_path):
    response = requests.get('https://swiss-po.ch/data/%s.pdb' % pdb_path)
