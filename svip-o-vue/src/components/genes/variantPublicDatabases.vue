                        ...
                        <div class="col-4">
                            <b-card>
                                ...
                                <table class='table table-sm table-hover'>
                                    ...
                                    <tbody>
                                        <tr v-for='(nb,d) in row.item.diseases' @click='row.item.filter=d'
                                            :class='(row.item.filter===d)?"pointer table-active":"pointer"'>
                                            <td>{{d}}</td>
                                            <td>{{nb}}</td>
                                        </tr>
                                    </tbody>
                        ...
                        <div class="col-8">
                            <b-card>
                                <h6 class='card-subtitle mb-2 text-muted'>Evidences <span
                                    class='float-right badge badge-primary' v-if='row.item.filter'
                                    style='font-size: 13px'>{{row.item.filter}} <button type="button"
                                                                                        class="close small ml-3"
                                                                                        aria-label="Close"
                                                                                        style='font-size: 14px'
                                                                                        @click='row.item.filter=""'><span
                                    aria-hidden="true">&times;</span></button></span></h6>
...
export default {
    ...
    methods: {
        ...
        normalizeItemList(items) {
            ...
            return items.split(",").map(x => x.trim()).join(", ");
        }
    },
    computed: {
        ...mapGetters({
        ...
        data() {
            ...
            _.forEach(this.variant.association_set, a => {
                ...
                data[source].diseases = data[source].diseases.concat(_.map(a.phenotype_set, a => {
                    return titleCase(a.term)
                }))
                data[source].database_evidences = data[source].database_evidences.concat(_.map(a.evidence_set, e => {
