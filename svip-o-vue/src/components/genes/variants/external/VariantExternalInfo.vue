<template>
  <div class="row">
    <Pathogenicity v-if="included('Pathogenicity') && mvInfo && (mvInfo.cadd || mvInfo.dbnsfp || extras)" :cadd="mvInfo.cadd" :dbnsfp="mvInfo.dbnsfp" :extras="extras" />
    <PopulationStats v-if="included('PopulationStats') && mvInfo" :mvInfo="mvInfo" />
    <SwissPO v-if="included('SwissPO')" :protein="variant.gene.symbol" :uniprot-id="variant.gene.uniprot_ids[0]" :change="variant.name" />
