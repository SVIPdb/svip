<template>
    <v-select
        ...
        :options="interactors" :value="value" label="label" @input="update"
        :reduce="(x) => x.label"
    ...
    >
        ...
        <template v-slot:selected-option-container="{ option, deselect, disabled, multiple }">
            <span :class="['selected-tag', 'vs__selected', `tag-${option.type}` ]" v-bind:key="option.index">
                ...
                <button v-if="multiple" :disabled="disabled" @click="deselect(option)" type="button" class="vs__deselect" aria-label="Remove option">
...
export default {
    ...
    computed: {
        interactors() {
            ...
            return [
                ...this.drugs.map(x => ({...x, type: 'drug', id: 'd_' + x.id, label: x.common_name })),
                ...this.genes.map(x => ({...x, type: 'gene', id: 'g_' + x.id, label: x.symbol }))
            ];
        }
    },
    methods: {
...
.tag-gene { background-color: #e79abd; }
.tag-drug { background-color: #8beea4; }
.entry-type {
    display: inline-block;
    border-radius: 5px; padding: 2px; border: solid 1px #ccc;
    color: #555;
    font-size: smaller;
}
</style>
