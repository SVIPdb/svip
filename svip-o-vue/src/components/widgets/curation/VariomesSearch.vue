    ...
    <b-card-body style="padding: 0">
        <b-row class="textmining-paginator">
            ...
            <b-col class="my-1 pager-search" md="2">
                <b-select v-model="filterCollection">
                    ...
                    <b-select-option :value="'medline'"
                        >medline</b-select-option
                    >
            ...
            <b-row class="search-params" align-v="center">
                <b-col class="d-inline-flex align-items-center">
                    ...
                    <v-select
                        ...
                        "
                        v-model="disease"
                        placeholder="select a disease to query"
                        :options="pub_diseases"
                        label="name"
                        :clearable="true"
                    >
        ...
        <div style="margin-top: 10px">
            ...
            <b-table
                ...
                :items="pubs_by_disease"
                thead-class="unwrappable-header"
                :sort-by="sortBy"
                :sort-desc="sortDesc"
                :filter="search"
            ...
            >
                ...
                <template v-slot:cell(score)="data">
                    <b class="dotted-line" :ref="data.item.id">{{
                        data.value.toFixed(2)
                    }}</b>
                    <b-tooltip :target="() => $refs[data.item.id]">
                        <pass :querydetails="data.item.details.query_details">
                            <ul
                                slot-scope="{ querydetails }"
                                v-if="querydetails"
                                class="p-0 m-0"
                            >
                                <li
                                    v-if="querydetails.query_gene_count"
                                    class="d-flex justify-content-between align-items-center gene"
                                >
                                    {{ preferredTerms.gene }}
                                    <span class="text-white pl-1">{{
                                        querydetails.query_gene_count.all
                                    }}</span>
                                </li>
                                <li
                                    v-if="querydetails.query_variant_count"
                                    class="d-flex justify-content-between align-items-center variant"
                                >
                                    {{ preferredTerms.variant }}
                                    <span class="text-white pl-1">{{
                                        querydetails.query_variant_count.all
                                    }}</span>
                                </li>
                                <li
                                    v-if="
                                        preferredTerms.disease &&
                                        preferredTerms.disease !== 'none' &&
                                        querydetails.query_disease_count
                                    "
                                    class="d-flex justify-content-between align-items-center disease"
                                >
                                    {{ preferredTerms.disease }}
                                    <span class="text-white pl-1">{{
                                        querydetails.query_disease_count.all
                                    }}</span>
                                </li>
                            </ul>
                ...
                <template v-slot:row-details="data">
                    <TransitionExpand>
                        ...
                        >
                            ...
                            <ul>
                                <li
                                    v-for="(data, facet_name) in data.item
                                        .details.facet_details"
                                    :key="facet_name"
                                >
                                    <b>{{ facet_name }}</b>
                                    <ul>
        ...
        <b-row>
            <b-col>
                ...
                >
                    ...
                    <div class="d-flex align-items-center">
                        For multi-variant search, batch querying, and many other
                        features, see the official interface:
                        <a
                            class="ml-1"
                            :href="`https://candy.hesge.ch/Variomes/?genvars=${variant.gene.symbol} (${variant.name})`"
                            target="_blank"
                            >Variomes</a
                        >
...
export default {
    ...
    computed: {
        ...
        combinedPubs() {
            if (!this.variomes || !this.variomes.publications) return [];
            ...
            return Object.values(this.variomes.publications).reduce(
                (acc, x) => acc.concat(x),
                []
            );
        },
        preferredTerms() {
            if (!this.variomes || !this.variomes.normalized_query) return {};
            ...
            return {
                gene:
                    nq.genes &&
                    nq.genes.length > 0 &&
                    (nq.genes[0].preferred_term || nq.genes[0].query_term),
                variant:
                    nq.variants &&
                    nq.variants.length > 0 &&
                    (nq.variants[0].preferred_term ||
                        nq.variants[0].query_term),
                disease:
                    nq.diseases &&
                    nq.diseases.length > 0 &&
                    (nq.diseases[0].preferred_term ||
                        nq.diseases[0].query_term),
            };
        },
        /**
        ...
        pub_diseases() {
            const disease_map = this.combinedPubs.reduce((diseases, pub) => {
                ...
                if (pub.details.facet_details.diseases) {
                    const disease_names =
                        pub.details.facet_details.diseases.map(
                            (x) => x.preferred_term
                        );
                    disease_names.forEach((x) => {
                        if (diseases[x] !== undefined) {
                            diseases[x] += 1;
                        } else {
                            diseases[x] = 1;
                        }
        ...
        pubs_by_disease() {
            if (!this.disease) return this.combinedPubs;

            return this.combinedPubs.filter((pub) =>
                pub.details.facet_details.diseases.some(
                    (y) => y.preferred_term === this.disease.name
                )
            );
        },
    },
    ...
    async created() {
        ...
        this.icdo_diseases = await HTTP.get(`/diseases?page_size=9999`).then(
