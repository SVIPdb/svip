        ...
        <b-collapse :visible="isSearchParamsVisible">
            <b-row class="search-params" align-v="center">
                <b-col class="d-inline-flex align-items-center">
                    <b>{{ $t("Disease:")}}</b>
                    <v-select
                        style="background: white; width: 450px; margin-left: 10px"
                        v-model="disease"
                        ...
                        <template v-slot:option="option">
                            <div style="display: flex; justify-content: space-between; margin: 5px">
                                ...
                                <div style="font-style: italic; color: #777; font-size: smaller">
                                    {{ option.count }}
                                </div>
                            </div>
                        </template>
                ...
                <template v-slot:row-details="data">
                    <TransitionExpand>
                        <div v-if="data.item._showDetails" class="sample-subtable tumor-subtable">
                            ...
                            <ul>
                                <li
                                    v-for="(data, facet_name) in data.item.details.facet_details"
                                    :key="facet_name">
                                    <b>{{ facet_name }}</b>
                                    <ul>
        ...
        <b-row>
            <b-col>
                <b-alert show variant="info" class="d-flex align-items-center justify-content-between mb-0">
                    ...
                    <div class="d-flex align-items-center">
                        {{ $t("For multi-variant search, batch querying, and many other features, see the official")}}
                        {{ $t("interface:")}}
                        <a
                            class="ml-1"
...
export default {
    ...
    computed: {
        ...
        preferredTerms() {
            ...
            return {
                gene:
                    nq.genes && nq.genes.length > 0 && (nq.genes[0].preferred_term || nq.genes[0].query_term),
                variant:
                    nq.variants &&
                    nq.variants.length > 0 &&
                    (nq.variants[0].preferred_term || nq.variants[0].query_term),
                disease:
                    nq.diseases &&
                    nq.diseases.length > 0 &&
                    (nq.diseases[0].preferred_term || nq.diseases[0].query_term),
            };
        },
        /**
        ...
        pubs_by_disease() {
            ...
            return this.combinedPubs.filter(pub =>
                pub.details.facet_details.diseases.some(y => y.preferred_term === this.disease.name)
            );
        },
    },
