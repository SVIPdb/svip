...
export default {
    ...
    computed: {
        summaryModel() {
            console.log(
                `summaryModel() changed: '${
                    this.showSummaryDraft ? this.summaryDraft : this.summary
                }'`
            );
    ...
    methods: {
        ...
        getSummaryDraft() {
            ...
            HTTP.get(
                `/gene_summary_draft/?gene=${this.gene.id}&owner=${this.user.user_id}`
            ).then((response) => {
        ...
        saveSummaryDraft() {
            ...
            if (this.serverSummaryDraft === null) {
                ...
                HTTP.patch(
                    `/gene_summary_draft/${this.serverSummaryDraft.id}`,
                    { content: this.summaryDraft }
                )
                    .then(() => {
                        this.getSummaryDraft();
        ...
        sendSummaryRequest(params) {
            ...
            if (!this.serverSummaryDraft) {
                ...
                HTTP.post(`/update_gene_summary`, params)
                    .then(() => {
                        console.log(
                            `summary after response: '${this.summaryModel}'`
                        );
                        this.serverSummaryDraft = null;
