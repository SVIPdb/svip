...
export default {
    ...
    methods: {
        ...
        highlighted: function(query, orig) {
            ...
            orig.replace(r, (match) => { parts.push(match); });
        ...
        hasSvipData: function(entry) {
            return svipVariants.find(
                (sv) => `${sv.gene_name} ${sv.variant_name}`.toLowerCase() === entry.label.toLowerCase()
            ) !== undefined;
        }
    }
}
