...
class CheckboxGroup extends PolymerElement {
  static get template() {
    return html`
      ...
      <div class$="{{_get_class_for_license_restrictions(changecnt)}}">In accordance with license, annotated entities listed below cannot be highlighted in text</div>
      <dom-repeat items="{{_get_category_list(changecnt)}}" as="cat" >
        <template>
          ...
          <dom-repeat items="{{_get_category_item_list(cat,changecnt)}}" >
            <template>
              <div>
                <input on-change="handle_checkbox_change"  type="checkbox" name="[[item.name]]" checked="[[item.checked]]">
  ...
  _dispatchEventStateChanged() {
    ...
    this.dispatchEvent(new CustomEvent('filter-state-change', {
      "composed": true,
      "detail": detail
    }));
  }
