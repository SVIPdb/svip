...
class PubmedSectionV2 extends PolymerElement {
  ...
  _handleOnMouseUp(e) {
    ...
    if (txt != '') {
      ...
      //console.log("Selected text is " + txt);
      this.dispatchEvent(new CustomEvent('passageselected', {
        composed: true,
        detail: {
          content: txt
        }
      ...
      if (el.title) {
        ...
        for (var i = 0; i < titleIds.length; i++) {
          ...
          if (id.startsWith("ann")) {
            this.dispatchEvent(new CustomEvent('passagetouched', {
              composed: true,
              detail: {
                id: id
              }
      ...
      for (var j = 0; j < ids.length; j++) {
        if (someId == ids[j]) {
          ...
          if (withScroll) {
            ...
            span.scrollIntoView({block:"center", behavior: "smooth", inline:"nearest"});
          }
