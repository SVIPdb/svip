...
class PubmedViewerV2 extends PolymerElement {
  static get template() {
    return html`
      ...
      <div class="viewer-hdr" >
        <span>
          <input type="checkbox" on-change="_handle_checkbox_change" checked="[[showlinks]]"><span>Show concept links</span>
        </span>
      ...
      <p>
        ...
        <span>[[publi.volume]]([[publi.issue]]):[[publi.medline_pgn]].</span>
        <template is="dom-if" if="{{_hasValue(publi.pmcid)}}">
          <span> - PMCID: </span><a target="_blank" href="https://www.ncbi.nlm.nih.gov/pmc/articles/[[publi.pmcid]]/">[[publi.pmcid]]</a>
        </template>
        <template is="dom-if" if="{{_hasValue(publi.pmid)}}">
          <span> - PMID: </span><a target="_blank" href="https://www.ncbi.nlm.nih.gov/pubmed/?term=[[publi.pmid]]">[[publi.pmid]]</a>
        </template>
        <template is="dom-if" if="{{_hasValue(publi.doi)}}">
          <span> - DOI: </span><a target="_blank" href="https://dx.doi.org/[[publi.doi]]">[[publi.doi]]</a>
        </template>
      </p>
      <p>Author(s):
      ...
      <dom-repeat items="[[publi.affiliations]]">
        <template>
            <div class="publi-affiliation-div"><span class="publi-affiliation-label">{{_getPmcaAffiliationLabel(index)}}</span> [[item.name]]</div>
        </template>
      </dom-repeat>
      </p>
      ...
      <dom-repeat items="[[publi.body_sections]]" >
        ...
        <template>
          ...
          <div class$="{{_getClassForSectionLabelAndTitle(item)}}">{{_getTextForSectionLabelAndTitle(item)}}
            <span class$="{{_getClassForDebug(debug)}}">id: [[item.id]]</span>
          </div>

          <pubmed-section-v2 nohighlight="[[nohighlight]]" showlinks="[[showlinks]]" debug="[[debug]]" data="[[item]]" fsccount="[[fsccount]]" filters="[[filters]]"></pubmed-section-v2>
        </template>
      ...
      <template is="dom-if" if="{{_hasFloatSections(publi)}}">
        ...
        <dom-repeat items="[[publi.float_sections]]" >
          ...
          <template>
            ...
            <pubmed-section-v2 nohighlight="[[nohighlight]]" debug="[[debug]]" data="[[item]]" fsccount="[[fsccount]]" filters="[[filters]]"></pubmed-section-v2>
          </template>
      ...
      <template is="dom-if" if="{{_hasBackSections(publi, showbacksections)}}">
  ...
  _hasBackSections(item) {
    ...
    return this.showbacksections && item.back_sections && item.back_sections.length > 0;
  }

  _getTextForSectionLabelAndTitle(item) {
    return item.title == "Title" ? "" : (item.label + " " + item.title).trim();
  }
      ...
      for (var s = 0; s < this.publi.body_sections.length; s++) {
        ...
        for (var c = 0; c < section.contents.length; c++) {
          ...
          for (var p = 0; p < searched_props.length; p++) {
            ...
            while (true) {
              var offset = textual_content.indexOf(search_value, offset + 1);
              if (offset == -1) break;
              var annot = this.createAnnotationFromSeedContentOffset(seed, cnt, offset); //console.log("adding", annot);
