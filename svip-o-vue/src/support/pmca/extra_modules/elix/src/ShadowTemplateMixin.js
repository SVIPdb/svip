...
export default function ShadowTemplateMixin(Base) {
    ...
    class ShadowTemplate extends Base {
        ...
        [symbols.render]() {
            ...
            const template = getPreparedTemplate(this); // Stamp the template into a new shadow root.
                    ...
                    {},
                    {
                        ...
                        get(target, property, receiver) {
                            if (!element.shadowRoot) {
                                ...
                                console.warn(
                                    `Tried to find shadow element "${property.toString()}" before the shadow root was rendered.`
                                );
                                return null;
                            }
...
function prepareTemplate(element) {
    ...
    if (!template) {
        /* eslint-disable no-console */
        console.warn(
            `ShadowTemplateMixin expects ${element.constructor.name} to define a property called [symbols.template].\nSee https://elix.org/documentation/ShadowTemplateMixin.`
        );
        return;
    }
    ...
    if (window.ShadyCSS && !window.ShadyCSS.nativeShadow) {
        ...
        const tag = element.localName; // @ts-ignore
