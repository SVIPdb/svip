...
function applyStateChanges(state, changes) {
  let result = false; // Applying the changes may produce a new round of changes, and that round
  ...
  for (let changed; changed = fieldsChanged(state, changes), !isEmpty(changed);) {
    ...
    Object.assign(state, changes); // Run the change handlers, gathering up the changes those produce.
    ...
    if (state[changeHandlersKey]) {
      state[changeHandlersKey].forEach(handler => {
        const {
          dependencies,
          callback
        } = handler; // Does this handler trigger on any of the changes we have?
