...
export default function FocusVisibleMixin(Base) {
  ...
  return class FocusVisible extends Base {
    ...
    get updates() {
      const base = super.updates || {}; // Suppress the component's normal `outline` style unless we know the
      ...
      const outline = base.style && base.style.outline || !this.state.focusVisible && 'none' || null;
      return merge(base, {
        style: {
          outline
        }
