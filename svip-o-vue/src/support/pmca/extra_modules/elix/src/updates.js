...
export function applyProperty(element, name, value) {
    if (!(name in element)) {
        ...
        console.warn(
            `Warning: attempted to set the "${name}" property of a ${nodeName}, which does not have such a property.`
        );
        return;
    }
    ...
    sources.forEach((source) => {
        if (source) {
            for (const key in source) {
                result[key] =
                    result[key] && isPlainObject(source[key]) // Merge object.
                        ? merge(result[key], source[key]) // No need to merge.
                        : source[key];
            }
        }
    });
