...
export default function SingleSelectionMixin(Base) {
  ...
  class SingleSelection extends Base {
    ...
    get canSelectNext() {
      ...
      return count === 0 ? false : this.state.selectionWraps || selectedIndex < 0 || selectedIndex < count - 1;
    }
    /**
    ...
    get defaultState() {
      ...
      state.onChange(['items', 'selectedIndex', 'selectionRequired'], (state, changed) => {
        ...
        if (items) {
          const validatedIndex = validateIndex(adjustedIndex, items.length, selectionRequired, selectionWraps);
          return {
    ...
    selectPrevious() {
      ...
      if (this.items && this.state.selectedIndex < 0 || this.state.selectionWraps && this.state.selectedIndex === 0) {
...
function updateSelectedIndex(element, selectedIndex) {
  const validatedIndex = validateIndex(selectedIndex, element.items.length, element.state.selectionRequired, element.state.selectionWraps);
