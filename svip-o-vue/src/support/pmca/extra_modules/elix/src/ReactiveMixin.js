...
export default function ReactiveMixin(Base) {
  return class Reactive extends Base {
    constructor() {
      // @ts-ignore
      super(); // Set the initial state from the default state defined by the component
      // and its mixins.
    ...
    [symbols.render]() {
      if (super[symbols.render]) {
        super[symbols.render]();
      }
    }
    /**
    ...
    render() {
      ...
      if (this[stateKey] !== previousState) {
        const firstRender = previousState == null; // Remember that we've rendered (or about to render) this state.

        this[renderedStateKey] = this[stateKey]; // If at least one of the setState calls was made in response to user
        ...
        this[symbols.render]();
        this[symbols.rendering] = false; // Let the component know it was rendered.
    ...
    async setState(changes) {
      ...
      Object.freeze(state); // Is this our first setState, or does the component think something's changed?
      ...
      this[stateKey] = state; // We only need to render if we're actually in the document.
      ...
      if (this.isConnected) {
        ...
        await Promise.resolve(); // Render the component.
