    ...
    <b-container fluid>
        ...
        <div v-else>
            ...
            <b-row>
                <b-col sm="9">
                    ...
                    <b-card no-body style="margin-bottom: 1.5em;">
                        <b-card-body>
                            ...
                            <div v-else-if="variomes && variomes.error" class="text-center">
                        ...
                        <b-card-body>
                            <b-container fluid>
                                <ValidationObserver ref="observer" tag="b-form" @submit.prevent>
                                    <ValidatedFormField v-slot="props" :modeled="form.variants" label="Add variants? (In case of a combination)" inner-id="variants-combination">
                                        <SearchBar id="variants-combination" variants-only multiple v-model="form.variants"/>
                                    </ValidatedFormField>

                                    <ValidatedFormField v-slot="props" :modeled="form.type_of_evidence" label="For which type of evidence?" inner-id="evidence" required>
                                        <b-form-select id="evidence" :required="required" v-model="form.type_of_evidence" @change="evidenceTypeChanged" :state="checkValidity(props)">
                                            <optgroup v-for="(label,index) in Object.keys(inputs)" :key="index" :label="label">
                                                <option v-for="(option,n) in Object.keys(inputs[label])" :key="n" :value="option">{{ option }}</option>
                                            </optgroup>
                                        </b-form-select>
                                    </ValidatedFormField>
                                    ...
                                    <ValidatedFormField v-slot="props" :modeled="form.effect" label="Effect of the variant on the therapy" inner-id="effect" required>
                                        <b-form-select id="effect" v-model="form.effect" :options="effects" :state="checkValidity(props)" />
                                    </ValidatedFormField>

                                    <ValidatedFormField v-slot="props" :modeled="form.tier_criteria" label="Select a Tier criteria" inner-id="tier_criteria" required>
                                        <b-form-select id="tier_criteria" v-model="form.tier_criteria" :options="tier_criteria" :state="checkValidity(props)" />
                                    </ValidatedFormField>

                                    <ValidatedFormField v-slot="props" :modeled="form.mutation_origin" label="Origin of the mutation" inner-id="mutation_origin" required>
                                        <b-form-select id="mutation_origin"
                                    ...
                                    <ValidatedFormField v-slot="props" :modeled="form.support" label="Support" inner-id="support" required>
                                        <b-form-select id="support"
                                    ...
                                    <ValidatedFormField v-slot="props" :modeled="form.comment" label="Personal comment" sublabel="(viewed by you only)" inner-id="comment">
                                    ...
                                    <b-form-group label="Your annotations" label-cols-sm="4" label-cols-lg="3">
                                        <span v-if="form.annotations && form.annotations.length > 0">
                                            <b-input-group v-for="(annotation,index) in form.annotations" :key="index" class="mt-3">
                                        ...
                                        <b-input-group v-else>
                                            <i class="text-muted">(Select text in the citation summary and right-click to add an annotation.)</i>
                                        </b-input-group>
                                    </b-form-group>
                                </ValidationObserver>
                            </b-container>
                    ...
                    <!--
                    <b-card v-if="this.form.id" class="shadow-sm mb-3" header-bg-variant="white">
                        <h5 slot="header" class="d-flex align-items-center">
                            History
                        </h5>

                        <b-card-body class="p-0 m-0">
                            <EvidenceHistory :entry_id="this.form.id" />
                        </b-card-body>
                    </b-card>
                    -->
                ...
                -->
                <b-col sm="3">
                    <b-button variant="outline-success" size="lg" block class="shadow-sm mb-3" :href="duplicateUrl" target="_blank">Duplicate</b-button>

                    <b-card class="shadow-sm mb-3" header-bg-variant="white" no-body>
                        <h5 slot="header" class="d-flex align-items-center">
                            Actions
                            <b-link class="ml-auto" :aria-expanded="showAction ? 'true' : 'false'" aria-controls="action" @click="showAction = !showAction">
                        ...
                        <b-card-body class="p-0 m-0">
                            <b-collapse id="action" v-model="showAction" class="m-3">
                                <ul class="submission_properties">
                                    <li>
                                        <icon class="mr-1" name="newspaper" />{{ source }}:
                                        <span class="value">
                                    ...
                                    <li>
                                        <icon class="mr-1" name="user" />Creator: <span class="value">{{this.form.owner_name || '-'}}</span>
                                    </li>
                                    <li>
                                ...
                                <div class="d-flex align-items-center">
                                    <b-link id="delete-btn" class="text-danger" :disabled="!this.form.id" @click="onDelete()">
                    ...
                    <b-card class="shadow-sm" header-bg-variant="white" no-body>
                        <h5 slot="header" class="d-flex align-items-center">
                            Keywords
                            <b-link class="ml-auto" :aria-expanded="showStat ? 'true' : 'false'" aria-controls="statistic" @click="showStat = !showStat">
                            ...
                            <b-collapse id="statistic" v-model="showStat" class="m-3">
                                <div v-if="variomes">
                                    <b-link v-bind="pubmedURL(`?term=${variomes.query.gene}[Title/Abstract]`)">
                                        <b-badge class="bg-gene">{{variomes.query.gene}} ({{variomes.publication.details.query_details.targetGeneCount}})</b-badge>
                                    </b-link>
                                    <b-link v-bind="pubmedURL(`?term=${variomes.query.variant}[Title/Abstract]`)">
                                        <b-badge class="bg-variant">{{variomes.query.variant}} ({{variomes.publication.details.query_details.targetVariantCount}})</b-badge>
                                    </b-link>
                                    <b-link v-bind="pubmedURL(`?term=${variomes.query.disease}[Title/Abstract]`)">
                                        <b-badge class="bg-disease">{{variomes.query.disease}} ({{variomes.publication.details.query_details.targetDiseaseCount}})</b-badge>
                                    </b-link>
                                    <b-link v-bind="pubmedURL(`?term=${variomes.query.gene}[Title/Abstract] AND ${variomes.query.variant}[Title/Abstract] AND ${variomes.query.disease}[Title/Abstract]`)">
                                        <b-badge class="bg-primary">{{variomes.query.gene}} + {{variomes.query.variant}} + {{variomes.query.disease}}</b-badge>
                                    </b-link>
                                    <b-link v-bind="pubmedURL(`?term=${variomes.query.gene}[Title/Abstract] AND ${variomes.query.variant}[Title/Abstract]`)">
...
const effect_set = Object.values(inputs).reduce((acc, x) => Object.assign(acc, x), {});
...
export default {
    ...
    methods: {
        ...
        doCopy() {
            this.$copyText(this.selection).then(
                function(e) { alert("Copied"); },
                function(e) { alert("Can not copy"); }
            );
        },
        evidenceTypeChanged() {
        ...
        rehydrate(data) {
            ...
            const {
                formatted_variants, last_modified,
                tier_level, tier_level_criteria,
            ...
            // extra variants
            const [main_variant, ...extra_variants] = formatted_variants;
            this.variant.id = main_variant.id;
        ...
        async submit(isDraft) {
            ...
            (this.is_saved
                ...
                .then((result) => {
                    this.$snotify.success(`${isDraft ? 'Draft' : 'Entry'} ${this.is_saved ? 'updated' : 'saved'}!`);
        ...
        onDelete() {
            if (confirm("Are you sure that you want to delete this entry?")) {
                HTTP.delete(`/curation_entries/${this.form.id}`)
                    .then((result) => {
                        // refresh curation lists on other pages
                        ...
                        this.form.id = null;
                        this.pageError = { title: "Entry Deleted", message: "You may now close this window/tab." }
                    })
            }
        },
        checkValidity(props) {
    ...
    computed: {
        ...
        effects() {
            ...
            //     : [];
            return this.form.type_of_evidence && effect_set[this.form.type_of_evidence]
    ...
    beforeRouteEnter(to, from, next) {
        store.dispatch("getGeneVariant", { variant_id: to.params.variant_id }).then(() => {
            next();
        });
    },
